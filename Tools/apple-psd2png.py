...
if save_as_psd:
    ...
    image = pixel_layer.compose(force=True, bbox=pixel_layer.bbox)
    image.save(output_path)

# save metadata to json database
res_width, res_height = pixel_layer.width, pixel_layer.height
left, top = mask_data.left - pixel_layer.left, mask_data.top - pixel_layer.top
right, bottom = mask_data.right - pixel_layer.right, mask_data.bottom - pixel_layer.bottom
if db_path is not None:
    ...
    color = sanitize_color(filename)
    db = db_load(db_path)
    if device_key in db:
        db[device_key]["images"][color] = filename
    else:
        db[device_key] = {
            "images": {
                color: filename
    ...
    # apply default color
    apply_default_color(db, device_key)
    # save changes
    db_save(db, db_path)
